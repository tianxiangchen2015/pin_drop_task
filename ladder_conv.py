...
train_fns_u, train_labels_u = load_unlabel_data()
# Load validation data
X_test_fns, test_labels = load_validation_data()
...
height_map_sizes = [499]
width_map_sizes = [40]
for c in range(1,C):
  height_map_sizes += [height_map_sizes[c-1]-2]
...
inputs = tf.placeholder(tf.float32, shape=(None, 499*40))
images = tf.reshape(inputs,[-1,499,40,1])
...
for c in range(C):
  if c == C-1: #Make the kernel as big as the final map sizes. Similar to Fully Convolutional Layer
    height = height_map_sizes[-1]
    width = width_map_sizes[-1]
    shape = [height,width,channel_sizes[c],channel_sizes[c+1]]
